...
export const saveDraft = ({ dispatch }, draft) => dispatch(
    'saveNote', {
        ...draft,
...
export const addDraftToDiscussion = ({ commit }, { endpoint, data }) => service
    ...
    .catch(() => {
        createAlert({
            message: __('ocorreu um erro ao adicionar um rascunho ao tópico.')
        });
    });

export const createNewDraft = ({ commit }, { endpoint, data }) => service
    ...
    .catch(() => {
        createAlert({
            message: __('ocorreu um erro ao adicionar um novo rascunho.')
        });
    });

export const deleteDraft = ({ commit, getters }, draft) => service
    ...
    .catch(() =>
        createAlert({
            message: __('ocorreu um erro ao excluir o comentário.'),
        })
    );

export const fetchDrafts = ({ commit, getters, state, dispatch }) => service
    ...
    .catch(() =>
        createAlert({
            message: __('ocorreu um erro ao buscar comentários pendentes.'),
        })
    );
...
export const updateDraft = (
    ...
    return service
        ...
        .catch(() =>
            createAlert({
                message: __('ocorreu um erro ao atualizar o comentário.')
            })
        );
};
...
export const scrollToDraft = ({ dispatch, rootGetters }, draft) => {
    ...
    const tab = draft.file_hash || (discussion && discussion.diff_discussion) ? CHANGES_TAB : SHOW_TAB;
    ...
    if (filePath) {
        dispatch('diffs/setFileCollapsedAutomatically', {
            filePath,
            collapsed: false
        }, {
            root: true
