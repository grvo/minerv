...
export default {
    i18n: {
        sectionTitle: s__('NorthstarNavigation|Redesenho da navegação'),
        toggleMenuItemLabel: s__('NorthstarNavigation|Nova navegação'),
        toggleLabel: s__('NorthstarNavigation|Alternar nova navegação'),

        updateError: s__(
            'NorthstarNavigation|Não foi possível atualizar a nova preferência de navegação. Por favor, tente novamente mais tarde.',
        )
    },
        ...
        async updateAndReload() {
            try {
                await axios.put(this.endpoint, {
                    user: {
                        use_new_navigation: this.isEnabled
                    }
                });
