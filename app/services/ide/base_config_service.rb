    ...
    class BaseConfigService < ::BaseService
        ...
        def check_access!
            unless can?(current_user, :download_code, project)
                raise ValidationError, 'permissões insuficientes para ler configurações'
            end
        end
        ...
        def load_config_content!
            ...
            unless config_content
                raise ValidationError, "falha ao carregar arquivo de configuração da web ide '#{WEBIDE_CONFIG_FILE}' para #{params[:sha]}"
            end
        end
