    ...
    module JiraConnect
        class Client < Gitlab::HTTP
            ...
            def send_info(project:, update_sequence_id: nil, **args)
                ...
                raise ArgumentError, 'argumentos inválidos' if responses.empty?

                responses.compact
            end

            # capturar informação de usuário para a conta fornecida
            ...
            def store_ff_info(project:, feature_flags:, **opts)
                ...
                handle_response(r, 'flags de feature') do |data|
            ...
            def metadata
                { providerMetadata: { product: "minerv #{Gitlab::VERSION}" } }
            end
            ...
            def handle_response(response, name, &block)
                ...
                if [200, 202].include?(response.code)
                    ...
                    when 401 then { 'errorMessages' => ['jwt inválido'] }
                    when 403 then { 'errorMessages' => ["o app não suporta #{name}"] }
                    when 413 then { 'errorMessages' => ['dado muito largo'] + data.map { |e| e['message'] } }
                    when 429 then { 'errorMessages' => ['limite rate excedido'] }
                    when 503 then { 'errorMessages' => ['serviço indisponível'] }
            ...
            def request_body_schema(response)
                ...
                'o body do request possui um json inválido'
            end
