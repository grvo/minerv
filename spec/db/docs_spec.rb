...
RSpec.shared_examples 'validar dicionário' do |objects, directory_path, required_fields|
    context 'para cada objeto' do
        ...
        # lista usada para fornecer exceções temporárias para categorias de recursos
        # que estão em transição e ainda não estão no arquivo feature_categories.yml
        ...
        it 'tem um arquivo de metadados' do
            expect(objects_without_metadata).to be_empty, multiline_error(
                'arquivos de metadados ausentes',

                objects_without_metadata.map {
                    |t| "  #{object_metadata_file(t)}"
                }
            )
        end

        it 'tem um arquivo de metadados válido' do
            expect(objects_without_valid_metadata).to be_empty, object_metadata_errors(
                'arquivos de metadados da tabela com erros',
        ...
        it 'tem uma categoria de recurso válida' do
            message = <<~TEXT.chomp
                por favor, use uma categoria de https://about.gitlab.com/handbook/product/categories/#categories-a-z

                arquivos de metadados de tabela com uma categoria de feição inválida
            TEXT
        ...
        it 'tem um arquivo de metadados válido com campos permitidos' do
            expect(objects_with_disallowed_fields).to be_empty, object_metadata_errors(
                'arquivos de metadados de tabela com campos não permitidos',
        ...
        it 'tem um arquivo de metadados válido sem campos ausentes' do
            expect(objects_with_missing_required_fields).to be_empty, object_metadata_errors(
                'arquivos de metadados de tabela com campos ausentes',
    ...
    def load_object_metadata(required_fields, object_name)
        ...
        begin
            ...
            result[:disallowed_fields] = "campos não permitidos: #{disallowed_fields.join(', ')}" unless disallowed_fields.empty?

            missing_required_fields = (required_fields - result[:metadata].reject { |_, v| v.blank? }.keys)

            unless missing_required_fields.empty?
                result[:missing_required_fields] = "faltando campos obrigatórios: #{missing_required_fields.join(', ')}"
            end
            ...
            if required_fields.include?(:feature_categories)
                ...
                if (invalid = invalid_feature_categories(object_feature_categories)).any?
                    result[:invalid_feature_category] = "categoria de recurso inválida: #{invalid.join(', ')}"
                end
            end
...
RSpec.describe 'vê a documentação', feature_category: :database do
    ...
    include_examples 'validar dicionário', views, directory_path, required_fields
end

RSpec.describe 'documentação de tabelas', feature_category: :database do
    database_base_models = Gitlab::Database.database_base_models.select { |k, _| k != 'geo' }
    ...
    include_examples 'validar dicionário', tables, directory_path, required_fields
end

RSpec.describe 'documentação de tabelas excluídas', feature_category: :database do
...
RSpec.describe 'documentação de visualizações excluídas', feature_category: :database do
